...
const regex = /const currentUserId = user\?\._id \|\| user\?\.id;\s*const otherUser = activeConversation\.participants\?\.find\(p => String\(p\._id \|\| p\) !== String\(currentUserId\)\);\s*if \(!otherUser\) return;/;

const replace = `const currentUserId = user?._id || user?.id;
